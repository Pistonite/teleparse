...
where
    F: FnOnce(&syn::DeriveInput) -> syn::Result<TokenStream2>,
{
...
where
    F: FnOnce(&syn::DeriveInput, A) -> syn::Result<TokenStream2>,
{
...
where
    F: FnOnce(&mut syn::DeriveInput) -> syn::Result<TokenStream2>,
{
...
where
    F: FnOnce(&mut syn::DeriveInput, A) -> syn::Result<TokenStream2>,
{
...
pub(crate) fn parse_crate_attr_meta(
    attr: &syn::Attribute,
) -> syn::Result<Punctuated<syn::Meta, syn::Token![,]>> {
...
pub(crate) fn from_result_keep_input(
    input: TokenStream2,
    result: syn::Result<TokenStream2>,
) -> TokenStream {
    match result {
        Ok(output) => output,
        Err(err) => {
...
pub(crate) fn parse_strip_root_meta_optional(
    input: &mut syn::DeriveInput,
) -> syn::Result<Option<Punctuated<syn::Meta, syn::Token![,]>>> {
    ...
    let root_attr = match ensure_one(root_attrs) {
        ...
        EnsureOne::More => syn_error!(
            &input.ident,
            "Multiple root {} attributes found! You might want to merge them.",
            CRATE
        ),
...
pub(crate) fn checked_regex_rule(input: &syn::LitStr) -> syn::Result<Regex> {
    ...
    if regex.find("").is_some() {
        syn_error!(
            input,
            "the rule regular expression must not match the empty string"
        );
    }
    Ok(regex)
}
