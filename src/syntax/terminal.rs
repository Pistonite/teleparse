...
impl<L: Lexicon, T: Terminal<L=L> + ToSpan + From<Token<L>> + 'static> AbstractSyntaxTree for T {
    ...
    fn produces_epsilon() -> bool {
        // a terminal always produces one token
        false
    }

    #[inline]
    fn check_left_recursive(_stack: &mut Vec<String>, _seen: &mut BTreeSet<TypeId>) -> Result<(), GrammarError> {
    ...
    #[inline]
    fn build_jump(_seen: &mut BTreeSet<TypeId>, _parsing: &mut Jump<Self::L>) {
