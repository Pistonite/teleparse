...
mod tests {
    ...
    fn parse_one() -> Result<(), GrammarError>{
        ...
        let result = parser.parse::<SplitTest>()?;
        assert_eq!(result, Some(SplitTest(tp::Split {
    ...
    fn parse_recover_expecting_item() -> Result<(), GrammarError>{
        ...
        assert_eq!(parser.info().errors, vec![
            syntax::Error::new(2..3, ErrorKind::Expecting(first_set!(T {Ident:*})))
        ]);
    ...
    fn parse_recover_skip_sees_separator() -> Result<(), GrammarError> {
        ...
        assert_eq!(parser.remaining(), "");
        assert_eq!(parser.info().errors, vec![
            syntax::Error::new(2..4, ErrorKind::Expecting(first_set!(T {Ident:*})))
        ]);
    ...
    fn parse_recover_expecting_item_skip_multiple() -> Result<(), GrammarError>{
        ...
        assert_eq!(parser.remaining(), "");
        assert_eq!(parser.info().errors, vec![
            syntax::Error::new(2..5, ErrorKind::Expecting(first_set!(T {Ident:*})))
        ]);
    ...
    fn parse_recover_follow() -> Result<(), GrammarError>{
        ...
        assert_eq!(result, Some(SplitTestFollow(tp::Split {
            elems: Node::new(0..3, vec![
                Ident::from_span(0..1),
                Ident::from_span(2..3),
            ]),
        ...
        assert_eq!(parser.remaining(), "");
        assert_eq!(parser.info().errors, vec![
            syntax::Error::new(4..6, ErrorKind::Expecting(first_set!(T {Ident:*})))
        ]);
    ...
    fn parse_split_option() -> Result<(), GrammarError>{
        ...
        assert_eq!(result, Some(SplitOfOption(tp::Split {
            elems: Node::new(0..5, vec![
    ...
    fn parse_split_same() -> Result<(), GrammarError>{
        ...
        assert_eq!(result, Some(SplitSame(tp::Split {
            elems: Node::new(0..9, vec![
                Ident::from_span(0..1),
                Ident::from_span(4..5),
                Ident::from_span(8..9),
            ]),
    ...
    fn split_option_same_conflict() {
        assert_not_ll1!(SplitOptionSame, GrammarError::FirstFollowSeqConflict(
    ...
    fn split_option_same_conflict_sep() {
        assert_not_ll1!(SplitOptionSepSame, GrammarError::FirstFollowSeqConflict(
            "(Ident)? Ident".to_string(),
            "(Ident)?".to_string(),
            "Ident".to_string(),
            "Ident".to_string()
    ...
    fn parse_partial_item_recover_autofill() -> Result<(), GrammarError> {
        ...
        assert_eq!(result, Some(Split2(tp::Split {
            elems: Node::new(0..10, vec![
                (ParenOpen::from_span(0..1), Ident::from_span(1..2), ParenClose::from_span(2..3)),
                (ParenOpen::from_span(4..5), Ident::from_span(5..6), ParenClose::from_span(6..6)),
                (ParenOpen::from_span(7..8), Ident::from_span(8..9), ParenClose::from_span(9..10)),
            ]),
            puncts: vec![
                OpAdd::from_span(3..4),
                OpAdd::from_span(6..7),
        ...
        assert_eq!(parser.remaining(), "");
        assert_eq!(parser.info().errors, vec![
            syntax::Error::new(6..7, ErrorKind::Expecting(first_set!(T {Paren:")"})))
        ]);
    ...
    fn parse_partial_item_recover_skip() -> Result<(), GrammarError> {
        ...
        assert_eq!(result, Some(Split2(tp::Split {
            elems: Node::new(0..9, vec![
                (ParenOpen::from_span(0..1), Ident::from_span(1..2), ParenClose::from_span(2..3)),
                (ParenOpen::from_span(6..7), Ident::from_span(7..8), ParenClose::from_span(8..9)),
            ]),
            puncts: vec![
                OpAdd::from_span(5..6),
            ]
    ...
    fn split_both_option_conflict() {
        assert_not_ll1!(SplitBothOption, GrammarError::FirstFollowSeqConflict(
            "(Ident)? ( (+)? (Ident)? )*".to_string(),
            "(Ident)?".to_string(),
            "( (+)? (Ident)? )*".to_string(),
            "Ident".to_string()
