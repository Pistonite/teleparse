    ...
    where P::Prod: Production<L = <T::Prod as Production>::L>,
{
    ...
    fn produce<'s>(
        ...
        let s = Self { span: Span::new(lo, hi), elems , puncts };
        (s, errors).into()
    }
}
...
mod tests {
    ...
    fn parse_one() -> Result<(), GrammarError>{
        ...
        let result = parser.parse::<PunctTest>()?;
        assert_eq!(result, Some(PunctTest(tp::Punct {
            span: Span::from(0..1),
    ...
    fn parse_stop_expecting_item() -> Result<(), GrammarError>{
        ...
        assert_eq!(result, Some(ParenPunct(
    ...
    fn parse_partial_item_recover_autofill() -> Result<(), GrammarError> {
        ...
        assert_eq!(result, Some(Punct2(tp::Punct {
            span: Span::from(0..10),
            elems: vec![
    ...
    fn parse_punct_same() -> Result<(), GrammarError>{
        ...
        assert_eq!(result, Some(PunctSame(tp::Punct {
            span: Span::from(0..9),
            elems: vec![
                Ident::from_span(0..1),
                Ident::from_span(4..5),
    ...
    fn punct_option_not_ll1() {
        assert_not_ll1!(PunctOfOption, GrammarError::FirstFirstConflict(
            "( (Ident)? )[+]*".to_string(),
            "( (Ident)? )[+]+".to_string(),
            "()".to_string(),
            "<empty>".to_string(),
    ...
    fn punct_option_sep_not_ll1() {
        assert_not_ll1!(PunctOfOptionSep, GrammarError::FirstFirstConflict(
            "((+)? ( Ident )[(+)?]*)?".to_string(),
            "(+)? ( Ident )[(+)?]*".to_string(),
            "()".to_string(),
            "<empty>".to_string(),
    ...
    fn punct_both_option_conflict() {
        assert_not_ll1!(PunctBothOption, GrammarError::FirstFollowSeqConflict(
            "( (Ident)? )[(+)?]+".to_string(),
            "(Ident)?".to_string(),
            "((+)? ( (Ident)? )[(+)?]*)?".to_string(),
            "Ident".to_string()
